...
class ResetPasswordValidateToken(GenericAPIView):#token
    """
    En esta API se verificara el token de recuperacion
    """
    throttle_classes = ()
    permission_classes = ()
    ...
    def post(self, request, *args, **kwargs):
        ...
        if reset_password_token is None:
            return Response({'Estado': 'Token no encontrado'}, status=status.HTTP_404_NOT_FOUND)
        ...
        if timezone.now() > expiry_date:
            ...
            return Response({'Estado': 'Expirado'}, status=status.HTTP_404_NOT_FOUND)
        
        return Response({'Estado': 'verificado'})


class ResetPasswordConfirm(GenericAPIView):#modificacion de contraseña
    """
    En esta API se realizara el cambio de contraseña en base al token de recuperacion
    """
    throttle_classes = ()
    permission_classes = ()
...
class ResetPasswordRequestToken(GenericAPIView): #Manda correo con token
    """
    En esta API se proveera de un metodo el cual pedira el correo electronico y generara una señal para crear el token de recuperacion
    """
    throttle_classes = ()
    permission_classes = ()
    ...
    def post(self, request, *args, **kwargs):
        ...
        if not active_user_found and not getattr(settings, 'DJANGO_REST_PASSWORDRESET_NO_INFORMATION_LEAKAGE', False):
            raise exceptions.ValidationError({
                'email': [_(
                    "Este correo no esta asociado a ningun usuario, verifique que este bien escrito")],
            })
